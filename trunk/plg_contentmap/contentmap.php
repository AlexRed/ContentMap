<?php
/**
 * @version $Id: contentmap.php  2008-05-05 18:42 
 * @ Viet Nguyen Hoang - viet4777 - www.xahoihoctap.net & www.luyenkim.net & www.turismo.eu $
 * @package     Joomla
 * @subpackage	Content
...
$enabled = JPluginHelper :: isEnabled  ('content','contentmap');	
/**
 * Content Plugin
 *
 * @package		Joomla
...
class plgContentcontentmap extends JPlugin
{
	 ...
	 * @since 1.5
	 */
	function plgContentcontentmap( &$subject, $params )
	{	
		parent::__construct( $subject, $params );
		$this->_plugin =& JPluginHelper::getPlugin('content','contentmap');
        ...
        // explicitely.
        //JPlugin::loadLanguage('plg_content_contentmap', JPATH_ADMINISTRATOR);	//Call language file	
        if (JPlugin::loadLanguage('plg_content_contentmap')) ;	//Call language file
        else JPlugin::loadLanguage('plg_content_contentmap', JPATH_ADMINISTRATOR);
        //JPlugin::loadLanguage('plg_content_contentmap',JPATH_ROOT . DS . 'plugins'. DS .'content'. DS .'contentmap'. DS .'i18n'. DS );	//Call language file        
...
/******************Plugin Parameters*******************************/   
		/**parametri aggiunti da ste*/
		$mapenabled = $this->params->get( 'mapenabled', '' );
		$streetviewenabled = $this->params->get( 'streetviewenabled', '' );
		$apikey = $this->params->get( 'apikey', '' );
		$mapwidth = $this->params->get( 'mapwidth', '' );
		$mapheight = $this->params->get( 'mapheight', '' );
		$zoom = $this->params->get( 'zoom', '' );
		$zoomlevel = $this->params->get( 'zoomlevel', '' );
...
/****************************************************************************/
                ...
                $doc->addStyleSheet($url.'plugins/content/contentmap/css/contentmap.css'); 
	if ($zoom==0){ // per decidere il tipo di zoom
		$zooms= 'GLargeMapControl()';
		} else{
	...
	if($script_tooltip==1) $overlib = 1; else $overlib = 0;
		...
		case 0: //Joomla mootool
			...
			var Tips2 = new Tips($$('.tooltip_contentmap'), { maxTitleChars: " . $tooltip_title_chars2 . ", fixed: false}, {
				initialize:function(){
		...
		if(($load_mootools = 'yes') && !$mainframe->get('loadMootoolslib')) {
			$doc->addScript($url.'plugins/content/contentmap/js/mootools.js');
		...
		default:
                ...
                $doc->addScript($url.'plugins/content/contentmap/js/dhtmltooltip.js');
                $doc->addStyleSheet($url.'plugins/content/contentmap/css/dhtmltooltip.css');
...
$content = '';
  ...
  /*INIZIO inserimento mappa singolo articolo - STE*/
$link="plugins/content/contentmap/css/googlemap.php";
$httpfile = file_get_contents($link) ;
if ($mapenabled==1 && $params->get('keyref')!= null){
	$content .= '<script src="http://maps.google.com/maps?file=api&amp;v=2&amp;key='.$apikey.'" type="text/javascript"></script>
<div id="gmap" style="text-align:center; width:'.$mapwidth.'px; height:'.$mapheight.'px;" >Google Map</div>';
	if ($streetviewenabled==1){
		$content .= '<br /><div id="pano" style="width: '.$mapwidth.'px; height: '.$mapheight.'px; background-image: url(plugins/content/contentmap/images/nostreetview.gif);background-repeat:no-repeat; background-position: center center;"></div>';
		} 
	$content .= '<div id="gmap2" style="width:'.$mapwidth.'px;" ><small>'.$httpfile.'</small></div>';
	$showtext_before_n_after = 1;
  } 
    /*FINE inserimento mappa singolo articolo - STE*/
/************Related news Items***********************/
...
/**********************************************=================***/
  ...
  foreach ( $rows as $row ){
	...
	$date_display = '';
	if($showdate) $date_display = '<span class="contentmap_date">'.$this->mygetdate($row->created,$offset,$showdateformat).'</span>';
	...
	$tip_desc_str = $this->strip_newline($tip_desc_str);
	...
	switch ($script_tooltip) {
	case 0:
		$linked_title_display = '<span class="tooltip_contentmap" Title="'.htmlspecialchars($title_tip_str).
		'::'.htmlspecialchars($image . "<div class='contentmap_tooltip'>". $tip_desc_str."</div>").'"><a href="'.$link.'">'.$title_str."</a>".'</span><br />'; 
	break;
	case 1:
		$text = htmlspecialchars(addslashes($image . "<div class='contentmap_tooltip'>". $tip_desc_str."</div>"));
		//$text = htmlspecialchars(addslashes($image));

		$tooltip_title = htmlspecialchars(addslashes($title_tip_str));
		$tooltip = " onmouseover=\"overlib('".$text."', CAPTION, '".$tooltip_title."', FGCOLOR, '".$tooltip_fgcolor."', BGCOLOR, '".$tooltip_bgcolor."', WIDTH,'".$tooltip_width."', HEIGHT,'".$tooltip_height."', BORDER, ".$tooltip_border.", CAPCOLOR, '".$tooltip_capcolor."', TEXTCOLOR, '".$tooltip_textcolor."');\" onmouseout=\"return nd();\" "; //assign mouseover text to put in title hyperlink
		$linked_title_display = '<a '.$tooltip.' href="'.$link.'">'.$title_str."</a>";
	break;
	default:
		//DHTML
		$linked_title_display = '<a href="'.$link.'" onmouseover="showttip(\''.addslashes(htmlspecialchars('<strong>'.strip_tags($row->title).'</strong><br/>'.$image . "<div class='contentmap_tooltip'>".$tip_desc_str."</div>")).'\', '.$tooltip_width.');" onmouseout="hidettip();">'.$title_str."</a>";//
...
if ($neweritemscount>0) {
 ...
 foreach ( $rows as $row ){
	...
	case 0:
		$linked_title_display = '<span class="tooltip_contentmap" Title="'.htmlspecialchars($title_tip_str).
		'::'.htmlspecialchars($image . "<div class='contentmap_tooltip'>". $tip_desc_str."</div>").'"><a href="'.$link.'">'.$title_str."</a>".'</span>';
	break;
	case 1:
		$text = htmlspecialchars(addslashes($image . "<div class='contentmap_tooltip'>". $tip_desc_str."</div>"));
		//$text = htmlspecialchars(addslashes($image));
	...
	$content .= "</li>\n";   
		...
		$link   = JRoute::_(ContentHelperRoute::getArticleRoute($row->slug, $row->catslug, $row->sectionid.":testset"));
		$apre_next = '<span class="contentmap_priviuospage"><a href="'. $link . '" title="'.$row->title.'">'.JText::_('LBL_PREV_PAGE').'</a></span>';
		$afirst = 0;
		}
...
/****************************************************************************/
    ...
    $article->text .= '<div style="margin-left:'.$marginleft.'; margin-right:'.$marginright.'; text-align:left;">';
		if($showtext_before_n_after) $article->text .= $textbefore.$content.$textafter;
    $article->text .= "</div>";
	} //Plugin On/off
	else return;
...
if(($charnum >0) && (JString::strlen($text_) > $charnum)) return JString::substr($text_,0,$charnum).(($more=='')?"&hellip;":$more);
else return $text_;
}
